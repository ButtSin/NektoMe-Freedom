...
class Tabs {
  ...
  constructor(rootElement, storageID) {
    ...
    this.statusElement = this.rootElement.querySelector(this.selectors.status);
    ...
    this.statusElementAnimation = new Animation(this.statusElementAnimationSettings);
  ...
  async initState() {
    let currentState = await this.settingsManager.getSessionTabState(this.storageID);
  ...
  async initUI() {
    ...
    afterVisualUpdate(() => (this.statusElementAnimation.duration = oldDuration));
  }
  ...
  getProxyState(initialState) {
    initialState = initialState ?? {
      ...
      statusElementInitialLeft: Math.floor(this.statusElement.getBoundingClientRect().left),
    };
  ...
  initScrollSettings(element, afterUiUpdate) {
    const setupEffects = () => {
      if (!element.perfectScrollbar) {
        element.perfectScrollbar = new PerfectScrollbar(element, this.perfectScrollbarSettings);
  ...
  fixFullVwLine() {
    ...
    elementsWithLine.forEach((element) => {
      let isTop = element.classList.contains('full-vw-line--completely-top') ? true : false;

      let borderWidth = isTop
  ...
  updateStatus() {
    ...
    if (this.statusElementAnimation.animationId) {
      ...
      this.statusElement.style.width = this.state.statusElementInitialWidth + 'px';
      this.statusElement.style.transform = `translateX(${getTranslate(this.statusElement, 'x')}px)`;
  ...
  animateStatus(progress) {
    const activeTabElementWidth = Math.round(this.state.activeTabElement.offsetWidth);
    const activeTabElementLeft = Math.floor(
      this.state.activeTabElement.getBoundingClientRect().left,
    );
    const activeTabElementOffsetLeft = Math.round(
      parseInt(getComputedStyle(this.state.activeTabElement).paddingLeft),
    );
    ...
    const statusElementFraction = (statusElementWidth - activeTabElementWidth) / statusElementWidth;
    ...
    if (leftDifference < 0) {
      transformTranslateX =
        statusElementLeft - activeTabElementOffsetLeft + leftDifference * -1 * progress;
    } else if (leftDifference > 0) {
      transformTranslateX =
        statusElementLeft - activeTabElementOffsetLeft - leftDifference * progress;
    } else transformTranslateX = 0;

    transformTranslateX = Math.floor(transformTranslateX);

    this.statusElement.style.left = 0;
    this.statusElement.style.transform = `translateX(${transformTranslateX}px) scaleX(${transformScaleX})`;
    ...
    if (progress >= 1 || leftDifference === 0) {
      ...
      this.statusElement.style.left = activeTabElementLeft - activeTabElementOffsetLeft + 'px';
    }
  }
  ...
  bindEvents() {
    ...
    this.navigationElement.addEventListener('keydown', (event) => this.onKeyDown(event));
  }
}
...
class TabsCollection {
  ...
  async init() {
    ...
    this.tabs = await Promise.all(
      Array.from(tabElements).map(async (element, index) => {
