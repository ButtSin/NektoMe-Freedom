...
class Tabs {
  ...
  initScrollSettings(element, afterUiUpdate) {
    const setupEffects = () => {
      if (!element.perfectScrollbar) {
        element.perfectScrollbar = new PerfectScrollbar(
          element,
          this.perfectScrollbarSettings,
        );
  ...
  animateStatus(progress) {
    ...
    const statusElementFraction =
      (statusElementWidth - activeTabElementWidth) / statusElementWidth;
    ...
    if (progress >= 1 || leftDifference === 0) {
      ...
      this.statusElement.style.left =
        activeTabElementLeft - activeTabElementOffsetLeft + "px";
    }
  }
