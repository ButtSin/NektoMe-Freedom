...
class Settings {
  selectors = {
    ...
    lightStyle: 'link[rel=stylesheet][media*=prefers-color-scheme][media*=light]',
  ...
  constructor() {
    ...
    this.themeSwitcherInputElements = this.themeSwitcherElement.querySelectorAll(
      this.selectors.themeSwitcherInput,
    );
    this.themeSwitchableElements = document.querySelectorAll(this.selectors.themeSwitchableElement);
    ...
    this.unlockSexFieldElement = document.querySelector(this.selectors.unlockSexFieldCheckbox);
    this.unlockCopyElement = document.querySelector(this.selectors.unlockCopyCheckbox);

    this.state = null;

    this.boundOnThemeSwitcherInputClick = this.onThemeSwitcherInputClick.bind(this);
  ...
  async initUI() {
    document.documentElement.classList.add(this.stateClasses.isAnimationDisabled);
    ...
    await afterVisualUpdate(() => {
      document.documentElement.classList.remove(this.stateClasses.isAnimationDisabled);
  ...
  async initState() {
    ...
    if (!safeTheme)
      safeTheme = [...this.themeSwitcherInputElements].find((input) => input.checked).value;
  ...
  fixThemeSwitcherIconPosition() {
    ...
    let inputBorderSize = parseFloat(
      getComputedStyle(this.themeSwitcherInputElements[0]).borderTopWidth,
    );
  ...
  bindEvents() {
    ...
    this.unlockCopyElement.addEventListener('change', (event) => this.boundOnCopyChange(event));
  }
}
