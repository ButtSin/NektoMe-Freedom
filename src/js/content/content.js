...
class SettingsManager {
  ...
  setLocalCopyUnlocked(copyUnlockedValue) {
    return this._setSetting('local', this.keys.common.copyUnlocked, copyUnlockedValue);
  }
  ...
  setLocalSexFieldUnlocked(sexFieldUnlockedValue) {
    return this._setSetting('local', this.keys.common.sexFieldUnlocked, sexFieldUnlockedValue);
  }
...
class SexFieldUnlocker {
  ...
  async initState() {
    ...
    if (this.sexFieldUnlocked === undefined) {
      this.sexFieldUnlocked = settingsManager.getDefaultSettings().sexFieldUnlocked;
    }
  ...
  setupSexFieldElements(main) {
    ...
    this.sexFieldElement = main ?? document.querySelector(this.selectors.sexFieldClass);
    this.sexButtonElements = Array.from(
      this.sexFieldElement.querySelectorAll(this.selectors.buttonsClass),
    );
    this.isInCommunicationTopic = this.sexButtonElements.length > MIN_COMMUNICATION_TOPIC_BUTTONS;
    this.ownSexButtonElements = this.sexButtonElements.slice(0, this.sexButtonElements.length / 2);
  ...
  async initSexFieldUnlockObserver() {
    return new Promise((resolve) => {
      ...
      if (this.observerNode) {
        ...
        this.sexFieldUnlockObserver.observe(this.observerNode, this.observerConfig);
      } else {
  ...
  setButtonStates(button) {
    ...
    const buttons = isOwn ? this.ownSexButtonElements : this.companionSexButtonElements;
    ...
    if (isOwn) {
      ...
      switch (originalIndex) {
        case 0:
          ...
          this.companionSexButtonElements[0].classList.add(this.stateClasses.checked);
          break;
        case 1:
        case 2:
          this.ownSexState.fill(false);
  ...
  enableButtons(buttons) {
    buttons.forEach((button) => button.classList.remove(this.stateClasses.disabled));
  }

  disableButtons(buttons) {
    buttons.forEach((button) => button.classList.add(this.stateClasses.disabled));
  }
  ...
  resetCompanionButtons() {
    ...
    if (this.ownSexState[0]) {
      this.companionSexButtonElements[1].classList.add(this.stateClasses.checked);
      this.companionSexButtonElements[0].classList.remove(this.stateClasses.checked);
    }

    if (this.ownSexState[1]) {
      this.companionSexButtonElements[0].classList.add(this.stateClasses.checked);
      this.companionSexButtonElements[1].classList.remove(this.stateClasses.checked);
    }
  }

  adjustCommunicationButtons() {
    const isCommunicationUpdate = this.isInCommunicationTopic && !this.isFirstUpdate;
    const ownButtonSomeoneChecked = this.ownSexButtonElements[0].classList.contains(
      this.stateClasses.checked,
    );
    const hasCompanionSelection = this.companionSexState[0] || this.companionSexState[1];
  ...
  updateUI() {
    ...
    this.ownSexButtonElements
      ...
      .forEach((button, index) =>
        button.classList.toggle(this.stateClasses.checked, this.ownSexState[index]),
      );

    this.companionSexButtonElements
      .slice(startIndex)
      .forEach((button, index) =>
        button.classList.toggle(this.stateClasses.checked, this.companionSexState[index]),
      );
  }
  ...
  onSexFieldClick(event) {
    if (!event.target.classList.contains(this.selectors.buttonBaseClass)) return;
  ...
  onSearchButtonClick(event) {
    const hasNoCompanionSelection = !this.companionSexState[0] && !this.companionSexState[1];
  ...
  onChromeStorageChange(event) {
    ...
    if (this.sexFieldUnlocked) {
      this.searchButtonElement = document.querySelector(this.selectors.searchButtonID);
      ...
      this.sexFieldUnlockObserver.observe(this.observerNode, this.observerConfig);
      this.searchButtonElement.addEventListener('click', this.boundOnSearchButtonClick, true);
      ...
      this.searchButtonElement.removeEventListener('click', this.boundOnSearchButtonClick);
      this.sexFieldElement.removeEventListener('click', this.boundOnSexFieldClick);
...
class CopyUnlocker {
  ...
  bindEvents() {
    chrome.storage.onChanged.addListener((changes) => this.onChromeStorageChange(changes));
