function setupVerticalShadow({
  ...
  shadow.style.width = `${container.clientWidth - containerPaddingRight - containerPaddingLeft}px`;
  shadow.style.height = `${(container.clientHeight * heightShadowPercentage) / 100}px`;
  ...
  if (shadowTop !== 'auto' && shadowBottom !== 'auto')
    throw new Error('Either shadowTop or shadowBottom must be set.');
  shadow.style.top = shadowTop === 'auto' ? 'auto' : `${shadowTop - containerPaddingTop}px`;
  shadow.style.bottom =
    shadowBottom === 'auto' ? 'auto' : `${shadowBottom - containerPaddingBottom}px`;

  if (shadowTop !== 'auto') container.prepend(shadow);
  else container.append(shadow);

  const scrollStartHidePosition = (container.scrollHeight / 100) * startHideShadowPercent;
  const scrollEndHidePosition = (container.scrollHeight / 100) * endHideShadowPercent;
