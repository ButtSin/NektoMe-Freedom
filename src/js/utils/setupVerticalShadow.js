function setupVerticalShadow({
  ...
  shadow.style.top =
    shadowTop === "auto" ? "auto" : `${shadowTop - containerPaddingTop}px`;
  shadow.style.bottom =
    shadowBottom === "auto"
      ? "auto"
      : `${shadowBottom - containerPaddingBottom}px`;

  if (shadowTop !== "auto") container.prepend(shadow);
  else container.append(shadow);

  const scrollStartHidePosition =
    (container.scrollHeight / 100) * startHideShadowPercent;
  const scrollEndHidePosition =
    (container.scrollHeight / 100) * endHideShadowPercent;
