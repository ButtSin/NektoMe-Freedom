...
const updateShadowsOpacity = async () => {
  ...
  if (isShadowUpdating || isThemeChanging.value) return;

  isShadowUpdating = true;
...
const collapsedUpdateShadowsOpacity = () => {
  if (rafId || isShadowUpdating) return;
...
const initParentEvents = () => {
  ...
  shadowAnimation = new Animation({
    ...
    draw: collapsedUpdateShadowsOpacity,
  });
  ...
  for (let event of parentEvents) {
    parentElement.addEventListener(event, parentEventsHandler, {
      passive: true,
    });
  }
};
...
watch(
  () => isShow,
  () => {
    if (isShow) collapsedUpdateShadowsOpacity();
  },
);
